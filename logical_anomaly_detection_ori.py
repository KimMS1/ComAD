...
for classname in classlist:
    train_file_path = os.path.join(sourcepath, f'{classname}_heat', 'train')
    trainfiles = sorted(glob.glob(os.path.join(train_file_path, '*')), key=safe_sort_key)

    tegood_file_path = os.path.join(sourcepath, f'{classname}_heat', 'test', 'good')
    tegoodfiles = sorted(glob.glob(os.path.join(tegood_file_path, '*')), key=safe_sort_key)

    telogical_file_path = os.path.join(sourcepath, f'{classname}_heat', 'test', 'logical_anomalies')
    telogicalfiles = sorted(glob.glob(os.path.join(telogical_file_path, '*')), key=safe_sort_key)

    testrufile_path = os.path.join(sourcepath, f'{classname}_heat', 'test', 'stru')
    testrufiles = sorted(glob.glob(os.path.join(testrufile_path, '*')), key=safe_sort_key)

    valfile_path = os.path.join(sourcepath, f'{classname}_heat', 'validation')
    valfiles = sorted(glob.glob(os.path.join(valfile_path, '*')), key=safe_sort_key)

    # for p in [train_file_path, tegood_file_path, telogical_file_path, testrufile_path, valfile_path]:
