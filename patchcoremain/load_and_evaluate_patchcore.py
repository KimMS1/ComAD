...
import patchcoremain.common
import patchcoremain.metrics
import patchcoremain.patchcore
import patchcoremain.sampler
import patchcoremain.utils

LOGGER = logging.getLogger(__name__)

_DATASETS = {"mvtec": ["patchcoremain.datasets.mvtec", "MVTecDataset"]}
...
def run(methods, results_path, gpu, seed, save_segmentation_images):
    ...
    device = patchcoremain.utils.set_torch_device(gpu)
    ...
    for dataloader_count, dataloaders in enumerate(dataloader_iter):
        ...
        patchcoremain.utils.fix_seeds(seed, device)
        ...
        with device_context:
            ...
            if save_segmentation_images:
                ...
                patchcoremain.utils.plot_segmentation_images(
                    results_path,
                    image_paths,
                    segmentations,
                    scores,
            ...
            auroc = patchcoremain.metrics.compute_imagewise_retrieval_metrics(
                scores, anomaly_labels
            )["auroc"]

            # Compute PRO score & PW Auroc for all images
            pixel_scores = patchcoremain.metrics.compute_pixelwise_retrieval_metrics(
    ...
    patchcoremain.utils.compute_and_store_final_results(
        results_path,
        result_scores,
...
def patch_core_loader(patch_core_paths, faiss_on_gpu, faiss_num_workers):
    def get_patchcore_iter(device):
        for patch_core_path in patch_core_paths:
            ...
            if n_patchcores == 1:
                nn_method = patchcoremain.common.FaissNN(faiss_on_gpu, faiss_num_workers)
                patchcore_instance = patchcoremain.patchcore.PatchCore(device)
                ...
                for i in range(n_patchcores):
                    nn_method = patchcoremain.common.FaissNN(
